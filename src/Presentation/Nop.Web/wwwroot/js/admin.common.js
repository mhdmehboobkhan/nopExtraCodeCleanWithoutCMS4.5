...
function WrapAndSaveBlockData(card, collapsed) {
  var hideAttribute = card.attr("data-hideAttribute");
    var url = 'preferences/savepreference'
    if ($('body').hasClass('admindashboard'))
        url = 'admin/preferences/savepreference'
    saveUserPreferences(rootAppPath + url, hideAttribute, collapsed);
}
...
function openModalPopup(url, type, data) {
    $.ajax({
        cache: false,
        type: type,
        url: url,
